...
if run_btn:
    try:
        ...
        with tabs[2]: # CHART (SMA + RSI + EVENTS)
            ...
            for dt, val in divs.items(): fig.add_annotation(x=dt, y=0, yref="paper", text="D", font=dict(color="gold"), showarrow=False, bgcolor="#1c2128")
        ...
        with tabs[3]: # VALUATION (DIVERSE CLUSTER)
            ...
            with v_l:
                ...
                st.markdown(f'<div class="valuation-box"><b>Mean P/B Valuation</b><br><span style="font-size:24px;">${round(info.get("bookValue",0)*1.85, 2)}</span></div>', unsafe_allow_html=True)
            with v_r:
        ...
        with tabs[4]: # AI THESIS (NEWS-ADAPTIVE)
            ...
            with b_l:
                ...
                <b>Topline Headline:</b> {news[0]['title'] if news else 'N/A'}<br><br>
                <b>Narrative:</b> Beyond the numbers, recent strategic pivots in the sector suggest that {ticker_sym} is entering a dominant compounding phase. Growth is being driven by high-margin segments that analysts believe will redefine the company's valuation floor.</div>""", unsafe_allow_html=True)
            with b_r:
                st.markdown(f"""<div class="thesis-box"><h4 style="color:#FF5252;">🔴 Bear Case (Systemic Risks)</h4>
                <b>Risk Headline:</b> {news[1]['title'] if len(news)>1 else 'N/A'}<br><br>
                <b>Narrative:</b> Valuation risk remains the primary concern. Critics argue the ROI on recent massive capex may face headwinds from regulatory scrutiny and vertical deceleration, potentially leading to a sharp multiple de-rating if earnings miss expectations.</div>""", unsafe_allow_html=True)
