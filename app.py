...
if run_btn:
    try:
        ...
        # --- HEADER (NO MORE DELTAGENERATOR CODE) ---
        ...
        c_head_2.markdown(f'### {info.get("longName", ticker_sym)} <span class="moat-badge wide-moat">WIDE MOAT</span>', unsafe_allow_html=True)
        c_head_2.markdown(f'<div style="font-size:32px; font-weight:800;">${info.get("currentPrice", 0):,.2f} <span style="font-size:18px; color:{"#4CAF50" if p_chg >= 0 else "#FF5252"};">{"▲" if p_chg >= 0 else "▼"} {abs(p_chg):.2f}%</span></div>', unsafe_allow_html=True)
        ...
        with tabs[4]: # AI THESIS (NEWS DRIVEN)
            st.subheader("Institutional Research Thesis")
            news = stock.news
            b1, b2 = st.columns(2)
            h1 = news[0].get('title', 'Positive Revenue Outlook') if news else 'Bullish Momentum'
            h2 = news[1].get('title', 'Supply Chain Headwinds') if len(news) > 1 else 'Market Risks'
