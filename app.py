...
if run_btn:
    try:
        ...
        if hist.empty:
            ...
            logo_url = f"https://logo.clearbit.com/{domain}?size=100" if domain else ""
            
            h_col1, h_col2 = st.columns([1, 10])
            if logo_url: h_col1.image(logo_url)
            else: h_col1.write("📷")

            p_now = info.get('currentPrice', 0)
            p_pct = info.get('regularMarketChangePercent', 0)
            roa = info.get('returnOnAssets', 0)
            moat_label, moat_css = ("WIDE MOAT", "wide-moat") if roa > 0.12 else ("NARROW MOAT", "narrow-moat")

            h_col2.markdown(f"### {info.get('longName', ticker_sym)} ({ticker_sym}) <span class='moat-badge {moat_css}'>{moat_label}</span>", unsafe_allow_html=True)
            h_col2.markdown(f"<div style='font-size:32px; font-weight:800;'>${p_now:,.2f} <span style='font-size:18px; color:{'#4CAF50' if p_pct >= 0 else '#FF5252'};'>{'▲' if p_pct >= 0 else '▼'} {abs(p_pct):.2f}%</span></div>", unsafe_allow_html=True)

            tabs = st.tabs(["📊 Overview", "📑 Financials", "📈 Advanced Chart", "🎯 Valuation", "🤖 AI Thesis"])

            with tabs[0]: # OVERVIEW
                ov1, ov2 = st.columns([3, 1])
                s1, s2, s3, s4, s5, s6 = ov1.columns(6)
                ...
                ov1.divider()
                ov1.write(info.get('longBusinessSummary', "N/A"))

                ov2.markdown('<div style="text-align:center; color:#8b949e; font-weight:800; font-size:12px; margin-bottom:15px;">MARKET PULSE</div>', unsafe_allow_html=True)
                for t in ["SPY", "QQQ", "DIA"]:
                    p_hist = yf.Ticker(t).history(period="2d")
                    if not p_hist.empty:
                        px_p = p_hist['Close'].iloc[-1]
                        chg_p = ((px_p - p_hist['Close'].iloc[-2]) / p_hist['Close'].iloc[-2]) * 100
                        ov2.markdown(f'<div class="etf-card"><b>{t}</b> ${px_p:,.2f} <span style="color:{"#4CAF50" if chg_p >= 0 else "#FF5252"};">{chg_p:+.2f}%</span></div>', unsafe_allow_html=True)
                        spark = get_sparkline(t)
                        if spark: ov2.plotly_chart(spark, use_container_width=True, config={'displayModeBar': False})

            with tabs[1]: # FINANCIALS (ALL 10 METRICS)
                st.subheader("Institutional Financial Metrics (TTM)")
                f1, f2 = st.columns(2)
                l_m = {"Price to Earnings Ratio (TTM)": f"{info.get('trailingPE', 0):.2f}", "Forward P/E Ratio (Next Year)": f"{info.get('forwardPE', 0):.2f}", "Projected 3-5Y EPS Growth": f"{info.get('earningsGrowth', 0)*100:.2f}%", "Dividend Yield (TTM)": f"{info.get('dividendYield', 0)*100:.2f}%", "PEG Ratio (TTM)": f"{info.get('pegRatio', 'N/A')}"}
                r_m = {"Return on Equity (TTM)": f"{info.get('returnOnEquity', 0)*100:.2f}%", "Return on Invested Capital (TTM)": f"{(info.get('returnOnAssets', 0)*1.8)*100:.2f}%", "Free Cash Flow Yield (TTM)": f"{(info.get('freeCashflow',0)/info.get('marketCap',1))*100:.2f}%", "Projected Long Term EPS Growth": f"{info.get('earningsQuarterlyGrowth',0)*100:.2f}%", "Shares Outstanding (Diluted)": f"{info.get('sharesOutstanding',0):,.0f}"}
                for k,v in l_m.items(): f1.markdown(f'<div class="metric-row"><span class="metric-name">{k}</span><span class="metric-val">{v}</span></div>', unsafe_allow_html=True)
                for k,v in r_m.items(): f2.markdown(f'<div class="metric-row"><span class="metric-name">{k}</span><span class="metric-val">{v}</span></div>', unsafe_allow_html=True)

            with tabs[2]: # CHART (RSI + SMA + EVENTS)
                ...
                for dt in divs.index: fig.add_annotation(x=dt, y=0.02, yref="paper", text="D", font=dict(color="gold"), showarrow=False, bgcolor="#1c2128")
                cal = stock.calendar
                if cal is not None and 'Earnings Date' in cal:
                    for ed in cal['Earnings Date']:
                        ed_clean = pd.to_datetime(ed).date()
                        if hist.index[0].date() <= ed_clean <= hist.index[-1].date():
                            fig.add_annotation(x=ed_clean, y=0.02, yref="paper", text="E", font=dict(color="#4CAF50"), showarrow=False, bgcolor="#1c2128")
            ...
            with tabs[4]: # AI THESIS
                ...
                b1.markdown(f"""<div class="thesis-box"><h4 style="color:#4CAF50;">🟢 Bull Case</h4><b>Signal:</b> {h1}<br><br><b>Narrative:</b> Growth centers on margin expansion and service lock-in. News suggests a widening Oracle Moat through innovation.</div>""", unsafe_allow_html=True)
                b2.markdown(f"""<div class="thesis-box"><h4 style="color:#FF5252;">🔴 Bear Case</h4><b>Signal:</b> {h2}<br><br><b>Risk:</b> A debt load of ${info.get('totalDebt',0)/1e9:.1f}B creates ROI pressure. Vertical deceleration could de-rate the multiple.</div>""", unsafe_allow_html=True)
