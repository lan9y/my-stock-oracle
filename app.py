...
def get_sparkline(ticker_code):
    try:
        ...
        fig.update_layout(xaxis_visible=False, yaxis_visible=False, margin=dict(l=0,r=0,t=0,b=0), height=40, width=100, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
        return fig
    except: return None
    ...
    try:
        ...
        with tabs[0]:
            ...
            with co2:
                ...
                for t in ["SPY", "QQQ", "DIA"]:
                    ...
                    st.markdown(f'<div class="etf-card"><b>{t}</b><br><span style="font-size:16px;">${px:,.2f}</span> <span style="color:{"#4CAF50" if chg >= 0 else "#FF5252"}; font-size:12px;">{chg:+.2f}%</span></div>', unsafe_allow_html=True)
                    st.plotly_chart(get_sparkline(t), config={'displayModeBar': False}, use_container_width=True)
        ...
        with tabs[2]:
            ...
            for dt, val in divs.items():
                fig.add_annotation(x=dt, y=0, yref="paper", text="D", font=dict(color="gold", size=10), showarrow=False, bgcolor="#1c2128", bordercolor="gold")
            ...
            if cal is not None and 'Earnings Date' in cal:
                for ed in cal['Earnings Date']:
                    ...
                    if hist.index[0].date() <= ed_c <= hist.index[-1].date():
                        fig.add_annotation(x=ed_c, y=0, yref="paper", text="E", font=dict(color="#4CAF50", size=10), showarrow=False, bgcolor="#1c2128", bordercolor="#4CAF50")
        ...
        with tabs[3]:
            st.subheader("Diverse Fair Value Clusters")
            ...
            with v_l:
                ...
                st.markdown(f'<div class="valuation-box"><b>VMI 20yr DCF Model</b><br><span style="font-size:24px;">${iv_20}</span></div>', unsafe_allow_html=True)
                st.markdown(f'<div class="valuation-box"><b>Mean P/B Valuation</b><br><span style="font-size:24px;">${iv_pb}</span></div>', unsafe_allow_html=True)
            with v_r:
                iv_peg = round(info.get('trailingEps', 1) * (info.get('earningsGrowth', 0.1)*100) * 1.5, 2)
                iv_graham = round(np.sqrt(22.5 * info.get('trailingEps', 1) * info.get('bookValue', 1)), 2)
                st.markdown(f'<div class="valuation-box"><b>PEG Fair Value</b><br><span style="font-size:24px;">${iv_peg}</span></div>', unsafe_allow_html=True)
                st.markdown(f'<div class="valuation-box"><b>Graham Number</b><br><span style="font-size:24px;">${iv_graham}</span></div>', unsafe_allow_html=True)
            
            st.divider()
            st.subheader("Wall Street Analyst Consensus")
            w1, w2, w3 = st.columns(3)
            w1.metric("Low Target", f"${info.get('targetLowPrice')}")
            w2.metric("Mean Target", f"${info.get('targetMeanPrice')}")
            w3.metric("High Target", f"${info.get('targetHighPrice')}")

        with tabs[4]:
            st.subheader(f"Data-Adaptive AI Thesis: {ticker_sym}")
            news = stock.news[:5]
            signals = [n['title'] for n in news]
            
            b_l, b_r = st.columns(2)
            with b_l:
                st.markdown(f"""<div class="thesis-box"><h4 style="color:#4CAF50;">🟢 Bull Case (Strategic Narratives)</h4>
                <b>Topline Signal:</b> {signals[0] if signals else 'Market capture expanding.'}<br><br>
                <b>Growth Story:</b> Analysts are highlighting the {info.get('revenueGrowth',0)*100:.1f}% revenue growth as a primary driver. Recent news indicates a pivot towards high-margin segments that may significantly expand the "Oracle Moat."</div>""", unsafe_allow_html=True)
            with b_r:
                st.markdown(f"""<div class="thesis-box"><h4 style="color:#FF5252;">🔴 Bear Case (Risk Narratives)</h4>
                <b>Risk Signal:</b> {signals[1] if len(signals)>1 else 'Valuation multiples under pressure.'}<br><br>
                <b>Constraint Story:</b> A debt load of ${info.get('totalDebt',0)/1e9:.1f}B coupled with rising interest coverage needs creates an ROI hurdle. Qualitative risks include regulatory scrutiny mentioned in global headlines regarding sector concentration.</div>""", unsafe_allow_html=True)
