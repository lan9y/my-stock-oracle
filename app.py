...
if run_btn:
    try:
        ...
        # --- CRITICAL: GLOBAL TIMEZONE STRIP ---
        if hist.index.tz is not None:
            hist.index = hist.index.tz_localize(None)

        # --- 1. HEADER (LOGO & MOAT) ---
        raw_web = info.get('website', '')
        # Domain cleaning for Logo API
        ...
        c_head_text.markdown(f'### {info.get("longName", ticker_sym)} <span class="moat-badge {moat_css}">{moat_label}</span>', unsafe_allow_html=True)
        c_head_text.markdown(f'<div style="font-size:32px; font-weight:800;">${p_now:,.2f} <span style="font-size:18px; color:{"#4CAF50" if p_chg >= 0 else "#FF5252"};">{"▲" if p_chg >= 0 else "▼"} {abs(p_chg):.2f}%</span></div>', unsafe_allow_html=True)
        ...
        with tabs[2]: # CHART (SMA + RSI + HARDENED EVENTS)
            ...
            # --- HARDENED EVENT MARKERS (SAFE COMPARISON) ---
            divs = stock.dividends
            if not divs.empty:
                divs.index = divs.index.tz_localize(None) if divs.index.tz else divs.index
                divs_in_range = divs[divs.index >= hist.index.min()]
                for dt in divs_in_range.index:
            ...
            try:
                cal = stock.calendar
                # Extract dates from calendar regardless of format (DataFrame or Dict)
                e_dates = []
        ...
        with tabs[4]: # AI THESIS (QUALITATIVE NEWS)
            ...
            news = stock.news
            b1, b2 = st.columns(2)
            h1 = news[0].get('title', 'Strategic Expansion') if news else 'Revenue Momentum'
            h2 = news[1].get('title', 'Operational Constraints') if len(news) > 1 else 'Valuation Pressures'
            
            b1.markdown(f"""<div class="thesis-box"><h4 style="color:#4CAF50;">🟢 Bull Case</h4>
            <b>Signal:</b> {h1}<br><br>
            <b>Growth Narrative:</b> The bull thesis centers on {ticker_sym}'s recent M&A synergies and their widening Moat in core services. Increased investment in high-margin verticals suggests significant topline expansion ahead.</div>""", unsafe_allow_html=True)
